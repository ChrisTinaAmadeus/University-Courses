...
def polynomial_data_simulator(
    ...
    data = {
        "train": [np.expand_dims(x_train, axis=1), y_train],
...
def visualization_kernel_curves(
    alpha: np.ndarray,
    x_train: np.ndarray,
    k_type: str,
    h: float,
    ...
    landmarks = 5 * (np.arange(0, 100) / 100 - 0.5)
    kappa = kernel(
        x=x_train, y=np.expand_dims(landmarks, axis=1), k_type=k_type, bandwidth=h
    )
    if normalize:
...
def kernel(
    x: np.ndarray, y: np.ndarray = None, k_type: str = "rbf", bandwidth: float = "h"
) -> np.ndarray:
    """
    ...
    if k_type == "rbf":
        dist2 = p_distance(y, x, p=2)
        K = np.exp(-dist2 / (bandwidth + 1e-12))
    elif k_type == "gate":
        dist1 = p_distance(y, x, p=1)
        K = (dist1 <= bandwidth).astype(float) / (bandwidth + 1e-12)
    elif k_type == "triangle":
        dist1 = p_distance(y, x, p=1)
        K = np.maximum(bandwidth - dist1, 0.0)
    elif k_type == "linear":
        K = y @ x.T
    else:
        raise ValueError("未知的核类型: {}。请重新输入！".format(k_type))
    return K


def nadaraya_watson_estimator(
    x_test: np.ndarray, x_train: np.ndarray, y_train: np.ndarray, k_type: str, h: float
) -> np.ndarray:
    """
    ...
    K = kernel(x=x_train, y=x_test, k_type=k_type, bandwidth=h)
    y_est = K @ y_train / (np.sum(K, axis=1, keepdims=True) + 1e-10)
    return y_est


# Task 2: Implement the training and the testing method of Kernel Ridge Regression method.
def training_krr(
    x_train: np.ndarray, y_train: np.ndarray, k_type: str, h: float, tau: float = 0.1
) -> np.ndarray:
    """
    ...
    K = kernel(x=x_train, y=x_train, k_type=k_type, bandwidth=h)
    N = x_train.shape[0]
    A = K + tau * np.eye(N)
    # Closed-form: alpha = (K + tau I)^{-1} y
    eigvals, eigvecs = np.linalg.eigh(A)
    alpha = eigvecs @ ((eigvecs.T @ y_train) / (eigvals[:, None] + 1e-12))
    return alpha


def testing_krr(
    x_test: np.ndarray, x_train: np.ndarray, alpha: np.ndarray, k_type: str, h: float
) -> np.ndarray:
    """
...
def training_krr_sgd(
    ...
    for ep in range(epoch):
        ...
        for start in range(0, num, batch_size):
            ...
            grad_data = (2.0 / max(1, batch_idx.shape[0])) * (K_b.T @ resid_b)
            grad_reg = 2.0 * tau * (K @ alpha)
            alpha = alpha - lr * (grad_data + grad_reg)

    return alpha


# Task 4: Inspired by the learning process of Lasso, develop a coordinate descent method to update alpha iteratively
def training_krr_bcd(
    x_train: np.ndarray,
    y_train: np.ndarray,
    k_type: str,
    h: float,
...
if __name__ == "__main__":
    ...
    for k in range(len(kernel_types)):
        for i in range(len(bandwidths)):
            ...
            bw = bandwidths[i]
            y_est0 = nadaraya_watson_estimator(
                x_test=data["test"][0],
            ...
            setting = "Kernel={}, Bandwidth={:.2f}".format(ker_type, bw)
